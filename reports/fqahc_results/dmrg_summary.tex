...
\(\texttt{partial\_*.jld2}\) 时的 legacy TiltedLat30 Fourier 网格。计算仍是
...
计算保持该数据原有的 legacy Fourier 网格和 hopping 约定不变。三重基态中，
...
旧 \(4\times6\) FCI 与当前 30-site 强耦合 CDW 的系统尺寸、填充和相互作用参数均不相同，
...
本节新增的结构因子与光电导使用同一个 \(V_1=100,V_2=V_3=0\) 参数点，并由
同一组三重 ED 基态得到，因此可以用 \(q=5,10\) 的 Bragg 峰确认三周期 CDW。
光电导峰仍只表示 \(q=0\)、同 sector 且电流矩阵元非零的中性激发；它不能单独
确定 many-body Chern number、分数量子化 Hall 响应、任意子性质或 phason。
这些问题还需要 flux insertion/charge pump、完整低能流形和尺寸标度联合判断。
