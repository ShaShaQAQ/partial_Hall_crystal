...
最低 15 态构成清楚隔离的候选低能流形。这里的“FQAHC 候选态”是对
这种低能流形与下述晶体序的工作命名。\(15=3\times5\) 的态数计数与周期三
平移破缺和五重分数拓扑简并的乘积相容，但零通量能谱本身还不能证明这种分解或
分数拓扑性质。
...
最低 8 个能级；右图放大候选低能流形。红点是最低 15 态，蓝灰点是其余保存的
...
\caption{温和参数点最低 15 态的静态结构因子。彩色细线是各 sector 的最低态，
...
有限 \(M\) 的参考。若要确认该候选态的分数拓扑性质，仍需结合 flux insertion 下
完整低能流形的谱流、many-body Chern number 或 charge pump，而不能由这些零通量
能谱、结构因子和纵向光电导单独判定。
