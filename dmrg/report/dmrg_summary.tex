...
The benchmark data file stores the exact and final Lanczos values used for all
four corrected component plots:
\begin{table}[H]
\centering
\small
...
The blue exact curves below are obtained from the complete finite-sector
eigenstate sum in Eq.~\eqref{eq:exact-resolvent}; they are not Lanczos data
drawn under a different label.  The final response-Lanczos result is drawn as a
thinner red dashed line, so gaps in the dashes expose the blue exact curve even
when the two agree at plotting resolution.  Each inset shows the pointwise
absolute exact--Lanczos difference on a logarithmic scale.
...
\end{subfigure}
\caption{All four corrected longitudinal optical-conductivity components.  In
every panel, the thick blue solid curve is the complete exact sum and the thin
red dashed curve is response Lanczos with \(M=892\); the inset is their
...
\caption{Scaled maximum error of the regular conductivity versus response
dimension.}
