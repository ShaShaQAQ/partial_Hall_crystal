...
本文档是 DMRG 分支持续更新的 LaTeX 结果汇总，用于记录基准数值、正式计算参数、
电荷泵浦图片以及简短的结果说明。生成的 DMRG 原始输出有意保存在 git 之外；
本文档只收录汇总数值和体积较小的图片。
...
当前 FQAHC/PHC 光学响应使用倾斜 30-site torus。完整模型与粒子数参数为
\[
...
对全部 sector 的能量排序后，最低流形实际包含 15 个近简并态，而不是 12 个；
\(N_p=12\) 是粒子数。最低 15 态覆盖全部 \(k=0,\ldots,14\)，每个 sector
恰好贡献一个。该流形的有限尺寸内部散布为
\[
  E_{15}-E_1=0.0287121891.
\]
第 16 个能级位于
\[
  E_{16}-E_1=0.2157939408,
\]
所以最低流形与更高能级之间的分离为
\[
  E_{16}-E_{15}=0.1870817517.
\]

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{figures/optical_response/phc30_manybody_spectrum.png}
\caption{当前光学参数点的 30-site 多体 ED 能谱。左图画出每个动量 sector
保存的最低 8 个能级；右图放大低能区。红色点是最低 15 态，它们在 15 个
sector 中各出现一次；蓝灰色点为其余保存能级。红色虚线标记 \(E_{15}-E_1\)，
蓝色点线标记 \(E_{16}-E_1\)。数据来自多节点 ED，而不是 response-Lanczos
光学迭代。}
...
\clearpage
\subsection{30-site FQAHC/PHC 正式响应计算}

大系统响应使用旧有的倾斜 30-site torus，保存的基态由多节点 ED 程序得到。
计算保持该数据原有的动量和 hopping 约定不变，并分别计算两个简并基态 sector
\(m=5\) 与 \(m=10\)。模型和响应参数为
\[
...
\centering
\small
\caption{30-site FQAHC/PHC response-Lanczos 计算的诊断量。}
\begin{tabular}{lrr}
\toprule
物理量 & \(m=5\) & \(m=10\) \\
\midrule
...
\begin{subfigure}{0.48\linewidth}
  ...
  \caption{\(\operatorname{Im}\sigma_{xx}^{\mathrm{total}}\).}
\end{subfigure}
\caption{30-site FQAHC/PHC 的纵向光电导。蓝色实线和红色虚线分别是独立计算的
...
旧 \(4\times6\) FCI 与当前 30-site FQAHC/PHC 的系统尺寸、填充和相互作用参数均不相同，
因此不能仅根据主峰由 \(4.107\) 变为 \(1.524\) 就判断某个相中的多体 gap 更小。
两套 \(\sigma_{xx}\) 直接给出的都是各自参数点上 \(q=0\) 的 current-active 中性激发及其
谱权重。旧 FCI 光谱还承担 exact sum-over-states 与 response-Lanczos 的数值基准作用；
30-site 结果则展示大 Hilbert 空间中无需全部激发态的 response-Lanczos ED 计算。

此外，现有 30-site 结构因子数据采用 \(V_1=10,V_2=V_3=2\)，而本节光电导采用
\(V_1=100,V_2=V_3=0\)。二者不是同一参数点，因而本文不使用前者的 CDW 强度或
Bragg 峰给当前光学峰作具体归属。仅凭 \(\sigma_{xx}\) 也不能单独确定拓扑基态简并、
many-body Chern number、分数量子化 Hall 响应、任意子性质、CDW 波矢或 phason；
这些问题需要与同一参数下的结构因子、flux insertion/charge pump、完整低能流形和
尺寸标度联合判断。
...
\begin{itemize}
  ...
  \item 在相同 30-site 参数下计算 \(S(\mathbf q)\)，并进一步补充
