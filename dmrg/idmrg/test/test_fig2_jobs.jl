...
@testset "Fig. 2 W003 production job contract" begin
    ...
    mpskit_gate_runner = joinpath(job_directory, "run_mpskit_gate.pbs")
    ...
    @test isfile(runner)
    @test isfile(mpskit_gate_runner)
    ...
    if isfile(mpskit_gate_runner)
        source = read(mpskit_gate_runner, String)
        @test success(`bash -n $mpskit_gate_runner`)
        @test occursin("#PBS -q cmt", source)
        @test occursin("#PBS -l nodes=1:ppn=24", source)
        @test occursin("#PBS -l walltime=01:00:00", source)
        @test occursin(production_repository, source)
        @test !occursin(retired_benchmark_repository, source)
        @test occursin(": \"\${MPSKIT_GATE:=restart}\"", source)
        @test occursin(": \"\${MPSKIT_GATE_THREADS:=4}\"", source)
        @test occursin("JULIA_NUM_THREADS=\"\$MPSKIT_GATE_THREADS\"", source)
        @test occursin("OMP_NUM_THREADS=1", source)
        @test occursin("OPENBLAS_NUM_THREADS=1", source)
        @test occursin("MKL_NUM_THREADS=1", source)
        @test occursin("audit_production_checkout.jl", source)
        @test occursin("test_mpskit_checkpoint.jl", source)
        @test occursin("IDMRG_TEST_ARTIFACTS", source)
        @test occursin("restart_gate.toml", source)
        @test occursin("before.toml", source)
        @test occursin("after.toml", source)
        @test occursin("state.h5", source)
        @test occursin("state-resumed.h5", source)
        @test occursin("state-next-flux.h5", source)
        @test occursin("sha256sum", source)
        @test occursin("/usr/bin/time -v", source)
        @test occursin("job_status.toml", source)
        @test occursin("/usr/bin/flock -n", source)
